...
#[test]
fn reads_refuse_v24_without_migration_but_append_migrates_it() {
    let state = private_state();
    json(invoke(&["--state-dir", path_text(state.path()), "--json"]));
    let database = state.path().join("hub.sqlite3");
    downgrade_empty_v25_to_v24(&database);
    ...
    assert_eq!(stored_version(&database), 25);
}
...
fn downgrade_empty_v25_to_v24(database: &Path) {
