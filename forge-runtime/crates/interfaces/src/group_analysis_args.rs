...
use crate::runtime_domain::{
    MAX_GROUP_MODEL_ANALYSIS_LIST_LIMIT, MAX_GROUP_MODEL_ANALYSIS_OUTPUT_TOKENS,
};

use super::{Command, GroupAnalysisCommand, GroupCommand, next_value, require_empty, usage};
...
pub(super) fn parse(
    ...
    match tokens.pop_front().as_deref() {
        ...
        Some(value) => Err(with_usage(&format!(
            "unknown group analysis command '{value}'"
        ))),
        None => Err(with_usage("group analysis command is required")),
    }
}
...
fn parse_prepare(
    ...
    let group_run_id = required_id(tokens, "group analysis prepare", "GROUP_RUN_ID")?;
    let mut model = None;
    ...
    while let Some(option) = tokens.pop_front() {
        match option.as_str() {
            ...
            _ => return Err(unknown("group analysis prepare", &option)),
        }
    }
    Ok(Command::Group(GroupCommand::Analysis(
...
fn parse_send(tokens: &mut VecDeque<String>) -> Result<Command, String> {
    let analysis_id = required_id(tokens, "group analysis send", "ANALYSIS_ID")?;
    let (confirm_off_machine, include_result) = parse_visibility_options(tokens, true)?;
    Ok(Command::Group(GroupCommand::Analysis(
...
fn parse_show(tokens: &mut VecDeque<String>) -> Result<Command, String> {
    let analysis_id = required_id(tokens, "group analysis show", "ANALYSIS_ID")?;
    let (_, include_result) = parse_visibility_options(tokens, false)?;
    Ok(Command::Group(GroupCommand::Analysis(
...
fn parse_visibility_options(
    ...
    while let Some(option) = tokens.pop_front() {
        match option.as_str() {
            ...
            _ => return Err(unknown("group analysis output", &option)),
        }
    }
    Ok((confirm, include))
}
...
fn parse_output_tokens(tokens: &mut VecDeque<String>) -> Result<u32, String> {
    ...
    if (1..=MAX_GROUP_MODEL_ANALYSIS_OUTPUT_TOKENS).contains(&parsed) {
        Ok(parsed)
    } else {
        Err(with_usage(&format!(
            "--max-output-tokens must be between 1 and {MAX_GROUP_MODEL_ANALYSIS_OUTPUT_TOKENS}"
        )))
    }
}
...
fn parse_limit(tokens: &mut VecDeque<String>) -> Result<usize, String> {
    ...
    if (1..=MAX_GROUP_MODEL_ANALYSIS_LIST_LIMIT).contains(&parsed) {
        Ok(parsed)
    } else {
        Err(with_usage(&format!(
            "--limit must be between 1 and {MAX_GROUP_MODEL_ANALYSIS_LIST_LIMIT}"
        )))
    }
}
