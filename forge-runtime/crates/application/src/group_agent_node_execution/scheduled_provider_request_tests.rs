...
fn list_validation_rejects_duplicates_and_unfiltered_rows() {
    ...
    sibling.scheduled_contract_sha256 = "c".repeat(64);
    sibling.scheduled_contract_id = format!(
        "scheduled-node-contract-{}",
        sibling.scheduled_contract_sha256
    );
    sibling.logical_request_sha256 = "d".repeat(64);
    sibling.logical_request_id =
        format!("scheduled-node-request-{}", sibling.logical_request_sha256);
    sibling.execution_ordinal = 1;
    sibling.node_id = "backend".into();
    sibling.prepared_request_sha256 = sibling.expected_sha256().expect("sibling digest");
    sibling.provider_request_id = format!(
        "scheduled-node-provider-request-{}",
        sibling.prepared_request_sha256
    );
    validate_list(&[record.clone(), sibling], Some(&record.graph_run_id), 2)
        .expect("two node requests may share one Run and schedule");
