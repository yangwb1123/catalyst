...
#[path = "scheduled_dispatch_release_source_validation.rs"]
...
pub(super) fn decode_release_control(
    ...
    GroupAgentScheduledNodeDispatchReleaseValidationError,
> {
    if json.is_empty() || json.len() > MAX_GROUP_AGENT_SCHEDULED_NODE_DISPATCH_RELEASE_CONTROL_BYTES
    {
        return Err(invalid(
            "scheduled release control input is outside its byte bound",
        ));
    }
    let value: GroupAgentScheduledNodeDispatchReleaseControl = serde_json::from_str(json)
        .map_err(|_| invalid("scheduled release control is invalid JSON"))?;
    value.validate()?;
    if value.canonical_json()?.as_bytes() != json.as_bytes() {
        return Err(invalid(
            "scheduled release control is not exact canonical JSON",
        ));
    }
    Ok(value)
}
...
pub(super) fn decode_authorization(
    ...
    GroupAgentScheduledNodeDispatchReleaseValidationError,
> {
    if json.is_empty() || json.len() > MAX_GROUP_AGENT_SCHEDULED_NODE_DISPATCH_AUTHORIZATION_BYTES {
        return Err(invalid(
            "scheduled dispatch authorization input is outside its byte bound",
        ));
    }
    let value: GroupAgentScheduledNodeDispatchAuthorization = serde_json::from_str(json)
        .map_err(|_| invalid("scheduled dispatch authorization is invalid JSON"))?;
    value.validate()?;
    if value.canonical_json()?.as_bytes() != json.as_bytes() {
        return Err(invalid(
            "scheduled dispatch authorization is not exact canonical JSON",
        ));
    }
    Ok(value)
}
...
pub(super) fn validate_release_control(
    ...
    let valid = control.v == GROUP_AGENT_SCHEDULED_NODE_DISPATCH_RELEASE_CONTROL_VERSION
    ...
    if !valid {
        return Err(invalid("invalid scheduled dispatch release-control header"));
    }
    source_validation::validate_sources(control)?;
    if control.expected_sha256()? != control.snapshot_sha256 {
        return Err(invalid(
            "scheduled dispatch release-control digest disagrees",
        ));
    }
    bounded(
        control.canonical_json()?.len(),
        MAX_GROUP_AGENT_SCHEDULED_NODE_DISPATCH_RELEASE_CONTROL_BYTES,
        "scheduled dispatch release control exceeds its byte bound",
    )
}
...
pub(super) fn validate_authorization(
    ...
    {
        return Err(invalid(
            "scheduled dispatch authorization identity disagrees",
        ));
    }
    bounded(
        value.canonical_json()?.len(),
        MAX_GROUP_AGENT_SCHEDULED_NODE_DISPATCH_AUTHORIZATION_BYTES,
        "scheduled dispatch authorization exceeds its byte bound",
    )
}
...
fn validate_authorization_header(
    ...
    let digests = authorization_digests(value);
    let valid = value.v == GROUP_AGENT_SCHEDULED_NODE_DISPATCH_AUTHORIZATION_VERSION
    ...
    valid
        .then_some(())
        .ok_or_else(|| invalid("invalid scheduled dispatch authorization header"))?;
    validate_shared_execution_policy(value)
}

fn valid_authorization_content_ids(value: &GroupAgentScheduledNodeDispatchAuthorization) -> bool {
    content_id(
        &value.schedule_id,
...
fn authorization_digests(value: &GroupAgentScheduledNodeDispatchAuthorization) -> [&str; 14] {
    [
...
fn validate_shared_execution_policy(
    ...
    super::super::validation::validate_provider(&provider)
        .map_err(|_| invalid("scheduled dispatch provider policy is invalid"))?;
    super::super::validation::validate_budgets(&value.budgets)
        .map_err(|_| invalid("scheduled dispatch budgets are invalid"))?;
    super::super::validation::validate_failure(&value.failure)
        .map_err(|_| invalid("scheduled dispatch failure policy is invalid"))?;
    ...
    valid
        .then_some(())
        .ok_or_else(|| invalid("scheduled dispatch execution policy disagrees"))
}
...
fn validate_authorization_policy(
    ...
    let valid = requirement.consent == GroupAgentScheduledNodeDispatchConsentRequirement::FreshOffMachine
        && requirement.consent_contract_version == GROUP_AGENT_SCHEDULED_NODE_DISPATCH_CONSENT_CONTRACT_VERSION
        && requirement.credential_preflight == GroupAgentScheduledNodeDispatchCredentialPreflight::HeaderSafeEnvironment
        && requirement.destination_preflight == GroupAgentScheduledNodeDispatchDestinationPreflight::ExactRegisteredDestination
        && requirement.pricing_preflight == GroupAgentScheduledNodeDispatchPricingPreflight::ExactSnapshotWithinMaxCost
        && requirement.project_lane_claim == GroupAgentScheduledNodeDispatchProjectLaneClaim::GlobalExclusiveUntilTerminal
        && requirement.provider_health_check == GroupAgentScheduledNodeDispatchProviderHealthCheck::Forbidden
        && requirement.atomic_transition == GroupAgentScheduledNodeDispatchAtomicTransitionRequirement::ExactPristineHeadAdmissionReleaseAndLaneClaim
        && requirement.successor == GroupAgentScheduledNodeSuccessorRequirement::VerifiedIntermediateTerminalReceiptBeforeSuccessor
    ...
    valid
        .then_some(())
        .ok_or_else(|| invalid("invalid passive scheduled dispatch release policy"))
}
...
fn validate_authorization_source_bindings(
    ...
    valid
        .then_some(())
        .ok_or_else(|| invalid("scheduled dispatch authorization source bindings disagree"))
}
...
fn validate_authorization_execution_bindings(
    ...
    valid
        .then_some(())
        .ok_or_else(|| invalid("scheduled dispatch authorization execution bindings disagree"))
}
