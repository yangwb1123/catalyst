...
#[test]
fn command_observation_and_binding_have_separate_identity_domains() {
    let base = adapt_fixture();

    let mut command = fixture().request;
    command.observation.command.cwd = "harness".into();
    let command_variant =
        adapt_canonical_request(canonical_request(&command).as_bytes()).expect("command variant");
    assert_ne!(command_variant.command_sha256, base.command_sha256);
    assert_ne!(
        command_variant.source_snapshot_sha256,
        base.source_snapshot_sha256
    );

    let mut observation = fixture().request;
    observation.observation.ended_at_unix_ms += 1;
    let observation_variant = adapt_canonical_request(canonical_request(&observation).as_bytes())
        .expect("observation variant");
    assert_eq!(observation_variant.command_sha256, base.command_sha256);
    assert_ne!(
    ...
    assert_eq!(binding_variant.command_sha256, base.command_sha256);
    assert_eq!(
...
fn exact_revalidation_rejects_resealed_structural_projection_drift() {
    ...
    collector.spec.collector.collector_id = "other-runner".into();
    cases.push(collector);
    let mut identity = adapted.evidence.clone();
    identity.metadata.record_id = "command-evidence-drift".into();
    cases.push(identity);
...
#[test]
fn unsealed_and_noncanonical_output_drift_are_rejected() {
    let adapted = adapt_fixture();
