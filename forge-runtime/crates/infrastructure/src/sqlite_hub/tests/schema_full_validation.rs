...
fn assert_valid_migration(fixture: (TempDir, std::path::PathBuf), seed_level: u8) {
    ...
    let connection = open_database(&database).expect("valid schema migrates to v21");
    assert_eq!(schema_version(&connection), 21);
    assert_seed_data(&connection, seed_level);
    drop(connection);

    let reopened = open_database(&database).expect("valid v21 schema reopens");
    assert_eq!(schema_version(&reopened), 21);
