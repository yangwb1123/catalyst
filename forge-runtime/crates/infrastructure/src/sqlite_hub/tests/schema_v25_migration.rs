...
#[test]
fn v24_data_survives_v25_migration_and_journal_starts_empty() {
    let fixture = exact_v24_fixture();
    let before = schedule_rows(&fixture.connection());

    let migrated = open_database(&fixture.database).expect("migrate exact v24 fixture to v25");
...
#[test]
fn malformed_current_v25_objects_are_rejected_without_repair() {
    assert_current_v25_rejected("malformed record-set bound", |connection| {
        let malformed = MIGRATE_V24_TO_V25_SQL.replacen("1048576", "1048575", 1);
        connection.execute_batch(&malformed)
    });
    assert_current_v25_rejected("missing appended index", |connection| {
        connection.execute_batch(MIGRATE_V24_TO_V25_SQL)?;
        connection.execute_batch(&format!("DROP INDEX {APPENDED_INDEX}"))
    });
    assert_current_v25_rejected("missing aggregate list index", |connection| {
        connection.execute_batch(MIGRATE_V24_TO_V25_SQL)?;
        connection.execute_batch(&format!("DROP INDEX {AGGREGATE_INDEX}"))
    });
    assert_current_v25_rejected("missing kind list index", |connection| {
        connection.execute_batch(MIGRATE_V24_TO_V25_SQL)?;
        connection.execute_batch(&format!("DROP INDEX {KIND_INDEX}"))
    });
    assert_current_v25_rejected("rogue object", |connection| {
        connection.execute_batch(MIGRATE_V24_TO_V25_SQL)?;
...
#[test]
fn final_validation_fault_rolls_v24_to_v25_back_atomically() {
    let fixture = exact_v24_fixture();
    let connection = fixture.connection();
...
fn assert_current_v25_rejected(
    name: &str,
