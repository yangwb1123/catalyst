...
fn injected_final_validation_failure_rolls_back_complete_v1_migration_chain() {
    ...
    let error = migrate_with_before_final_fault_for_test(&connection, |migrated| {
        ...
        assert_eq!(schema_version(migrated), 18);
    ...
    .expect_err("real final v18 validation must reject the injected rogue table");
    assert!(
        reached_final.get(),
    ...
    assert_eq!(
        message, "Hub v18 main catalog has invalid object inventory",
        "error must originate from the real final v18 catalog validator"
    );
