...
fn active_v4_claim_and_old_schema_survive_v13_migration_and_reopen() {
    ...
    let connection = open_database(&database).expect("active v12 Hub migrates to v13");
...
#[test]
fn failed_final_validation_rolls_back_v12_to_v13_atomically() {
    let (root, database) = legacy_active_v12_database();
...
fn old_schema(snapshot: &[SchemaRow]) -> Vec<SchemaRow> {
    snapshot
        .iter()
        .filter(|(_, name, _, _)| name != SCHEDULE_TABLE && name != SCHEDULE_INDEX)
        .cloned()
        .collect()
}
