pub(super) const MIGRATE_V13_TO_V14_SQL: &str =
    "CREATE TABLE group_agent_graph_scheduled_node_contract_candidates (
  ...
  contract_scope TEXT NOT NULL
    CHECK(typeof(contract_scope) = 'text'
      AND contract_scope = 'schedule_initial_node_only'),
...
CREATE INDEX group_agent_graph_scheduled_node_candidates_project_lane
  ON group_agent_graph_scheduled_node_contract_candidates(
    project_lane_sha256, created_at_ms DESC, id DESC
  );
CREATE INDEX group_agent_graph_scheduled_node_candidates_created
