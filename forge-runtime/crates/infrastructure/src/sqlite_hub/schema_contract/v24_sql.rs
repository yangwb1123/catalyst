// SQLite v24 migration: make the persisted successor-candidate bound match
// the 8 MiB cross-language scheduled-contract bound. The initial candidate
// remains content-free and cannot approach its existing 4 MiB storage bound.
pub(super) const MIGRATE_V23_TO_V24_SQL: &str =
    "CREATE TABLE group_agent_graph_scheduled_node_successor_candidates_v24 (
  ...
  execution_ordinal INTEGER NOT NULL
    CHECK(typeof(execution_ordinal) = 'integer' AND execution_ordinal BETWEEN 1 AND 31),
  ...
  predecessor_receipt_count INTEGER NOT NULL
    CHECK(typeof(predecessor_receipt_count) = 'integer'
      AND predecessor_receipt_count BETWEEN 0 AND 31
      AND predecessor_receipt_count = required_predecessor_node_count),
  ...
  contract_blob BLOB NOT NULL
    CHECK(typeof(contract_blob) = 'blob'
      AND length(contract_blob) BETWEEN 1 AND 8388608),
  contract_bytes INTEGER NOT NULL
    CHECK(typeof(contract_bytes) = 'integer'
      AND contract_bytes BETWEEN 1 AND 8388608
...
INSERT INTO group_agent_graph_scheduled_node_successor_candidates_v24
  SELECT * FROM group_agent_graph_scheduled_node_successor_candidates;
DROP TABLE group_agent_graph_scheduled_node_successor_candidates;
ALTER TABLE group_agent_graph_scheduled_node_successor_candidates_v24
  RENAME TO group_agent_graph_scheduled_node_successor_candidates;
