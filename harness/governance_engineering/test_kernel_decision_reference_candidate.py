#!/usr/bin/env python3
"""Regressions for ADR-0090/0091 decision reference governance."""
...
class KernelDecisionReferenceCandidateTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_registry_is_v39_scope_neutral_structural_candidate(self):
        ...
        self.assertEqual(
            CONTRACT,
            self.policy["kernel_decision_reference_core_v1_candidate_contract"],
        )
        ...
        self.assertEqual(22, len(ATTESTATIONS))
    ...
    def test_refs_pins_and_three_language_projection_split_are_exact(self):
        ...
        for digest in (PYTHON_MANIFEST_SHA256, CORE_MANIFEST_SHA256,
                       GO_MANIFEST_SHA256, RUST_FLAT_MANIFEST_SHA256):
            self.assertEqual(64, len(digest))
        self.assertEqual(9, len(RUST_SHA256))
        self.assertIn("source_distributed", REFERENCE_IMPLEMENTATIONS[
            "kernel_decision_reference_core_v1_python"]["projection"])
        for language in ("go", "rust"):
            self.assertIn("not_scaffolded", REFERENCE_IMPLEMENTATIONS[
                f"kernel_decision_reference_core_v1_{language}"]["projection"])
    ...
    def test_catalyst_exact13_go_parity_is_frozen(self):
    ...
    def test_catalyst_flat_exact9_rust_and_registration_are_frozen(self):
    ...
    def test_strict_proposed_decisions_and_narrow_roadmap_are_pinned(self):
        ...
        roadmap.write_text(roadmap.read_text(encoding="utf-8").replace(
            "- [ ] 完成 Governance Kernel/PDP", "- [x] 完成 Governance Kernel/PDP"),
            encoding="utf-8")
        self.assertTrue(any("must remain open" in issue
    ...
    def test_scope_argv_authority_and_distribution_drift_fail_closed(self):
        ...
        mutated["scope"]["shipped_evaluators"].append(
            "kernel_decision_reference_core_v1")
        block = mutated["kernel_decision_reference_core_v1_candidate_contract"]
        block["attestations"]["authority_attestation"] = True
        block["effective_semantics"]["effective_hardness"] = "required"
        ...
        mutated["reference_implementations"][
            "kernel_decision_reference_core_v1_go"]["projection"] = "universal"
    ...
    def test_physical_package_and_parity_drift_fail_closed(self):
        ...
        for package_name, checker, filename in (
            ...
            with self.subTest(package=package_name, mutation="extra"):
                root, target = self._parity_tree(package_name)
                (target / "extra.file").write_text("x")
                self.assertTrue(any("lexical closure" in issue for issue in checker(root)))
        if (self.repo / RUST_PACKAGE).is_dir():
            for mutation in ("missing", "duplicate"):
                with self.subTest(package=RUST_PACKAGE, registration=mutation):
                    ...
                    line = "pub mod kernel_decision_contract;"
