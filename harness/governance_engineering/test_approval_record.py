"""ADR-0059 ApprovalRecord governance integration tests."""
...
from governance_engineering.approval_record import (  # noqa: E402
    APPROVAL_RECORD,
    PROMOTION_MARKERS,
    RESULT,
    detector_issues,
...
class ApprovalRecordGovernanceTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_accepted_registry_detector_route_and_skill_are_contract_only(self):
        self.assertEqual(self.policy["approval_record"], APPROVAL_RECORD)
        ...
        self.assertEqual(self.policy["approval_record"]["positive_result"], RESULT)

    def test_schema_and_fixture_are_frozen(self):
        self.assertEqual(schema_issues(self.root), [])
        self.assertEqual(fixture_issues(self.root), [])

    def test_approval_record_cannot_be_runtime_or_planned(self):
        mutated = copy.deepcopy(self.policy)
        mutated["scope"]["shipped_kinds"].append("ApprovalRecord")
        mutated["scope"]["planned_kinds"].append("ApprovalRecord")
        ...
        self.assertTrue(any("cannot remain planned" in issue for issue in issues))

    def test_authority_escalation_is_rejected(self):
        mutated = copy.deepcopy(self.policy)
        mutated["approval_record"]["unavailable_runtime"][
            "effective_approval"] = "available"
        mutated["approval_record"]["assessment_constants"][
            "permission_attestation"] = True
        mutated["approval_record"]["production_effects"] = "allowed"
        issues = registry_issues(mutated, self.policy_path)
    ...
    def test_detector_cannot_become_load_bearing(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            detector = next(item for item in data["detectors"]
                            if item["id"] == "governance.approval_record_contract")
    ...
    def test_accepted_promotion_markers_cannot_regress(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            decision = root / "docs/adr/0059-approval-record-v1-contract-only.md"
            decision.write_text("- Status: Candidate", encoding="utf-8")
            issues = promotion_issues(root)
            self.assertEqual(len(issues), 1)
            self.assertIn("missing accepted ADR-0059 marker", issues[0])
    ...
    def test_optional_promotion_facts_skip_only_when_entirely_absent(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            relative, marker = next(
                ...
                if item[0] != "docs/adr/0059-approval-record-v1-contract-only.md"
            )
            target = root / relative
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_text(marker, encoding="utf-8")
            ...
            self.assertTrue(all("cannot validate ADR-0059 promotion" in issue
                                for issue in issues))
