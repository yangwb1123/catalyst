#!/usr/bin/env python3
"""CLI for ADR-0056 golden and exact declared-assessment checks."""
...
from capability_grant_contract import (ContractError, decode_assessment, decode_request,
                                        load_golden, validate_assessment)
from capability_grant_contract.canonical import read_bounded_file
from capability_grant_contract.constants import MAX_ASSESSMENT_BYTES, MAX_REQUEST_BYTES


def validate_golden_fixture(repo_root: Path) -> list[str]:
    """Return governance integration issues instead of raising."""
    try:
        load_golden(repo_root)
    except ContractError as error:
        return [f"CapabilityGrant v1 golden invalid: {error}"]
    return []


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="validate CapabilityGrant v1 declarations")
...
def _run(args: argparse.Namespace) -> None:
    if args.golden_root is not None:
        ...
        print("CapabilityGrant v1 golden: OK (declarations only; no authority)")
        return
    ...
    print("CapabilityGrant v1 instance: OK (declarations only; no authority)")
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"CapabilityGrant v1: ERROR: {error}", file=sys.stderr)
        return 1
    return 0
