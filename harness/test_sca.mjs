...
// --- parseManifest: Cargo.lock ([[package]] blocks) ---------------------------
test('parseManifest parses Cargo.lock [[package]] blocks', () => {
  const lock = [
  ...
  const deps = parseManifest(lock, 'Cargo.lock');
  assert.deepEqual(deps, [
    { name: 'tokio', version: '1.41.1', ecosystem: 'crates.io' },
    { name: 'rusqlite', version: '0.32.1', ecosystem: 'crates.io' },
  ], '[[package]] blocks parsed; [metadata] tail ignored');
});
