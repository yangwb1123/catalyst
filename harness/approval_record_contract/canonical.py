"""Strict bounded canonical JSON for the pure ApprovalRecord contract."""
...
from .constants import MAX_ARRAY, MAX_DEPTH, MAX_FIELDS, MAX_STRING_BYTES

MAX_CANONICAL_BYTES = 4_194_304


class ContractError(ValueError):
    """Caller supplied bytes do not satisfy the frozen contract."""
...
def canonical_json(value: Any) -> bytes:
    _measure_bounded(value, MAX_CANONICAL_BYTES)
...
def _measure_string(value: str, remaining: list[int]) -> None:
    ...
    escapes = value.count('"') + value.count("\\")
    _consume(2 + len(encoded) + escapes, remaining)
...
def bounded_digest(domain: bytes, value: Any, maximum: int, label: str) -> str:
    payload = bounded_canonical_json(value, maximum, label)
