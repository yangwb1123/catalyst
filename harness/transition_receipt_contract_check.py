#!/usr/bin/env python3
"""CLI for ADR-0060 golden and exact declared-assessment checks."""
...
from transition_receipt_contract import (ContractError, decode_assessment, decode_request,
                                          load_golden, validate_assessment)
from transition_receipt_contract.canonical import read_bounded_file
from transition_receipt_contract.constants import MAX_ASSESSMENT_BYTES, MAX_REQUEST_BYTES


def validate_golden_fixture(repo_root: Path) -> list[str]:
    """Return contract issues without minting Transition authority."""
    try:
        load_golden(repo_root)
    except ContractError as error:
        return [f"TransitionReceipt v1 golden invalid: {error}"]
    return []


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="validate TransitionReceipt v1 declarations")
...
def _run(args: argparse.Namespace) -> None:
    if args.golden_root is not None:
        ...
        print("TransitionReceipt v1 golden: OK (declarations only; no transition authority)")
        return
    ...
    print("TransitionReceipt v1 instance: OK (declarations only; no transition authority)")
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"TransitionReceipt v1: ERROR: {error}", file=sys.stderr)
        return 1
    return 0
