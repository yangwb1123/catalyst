"""Golden validation for the command observation Evidence adapter v1."""
...
from .adapter import (adapt_request, compute_command_digest, compute_request_digest,
                      compute_source_digest, validate_evidence_record, validate_request)
from .codec import (canonical_command, canonical_json, canonical_observation,
                    decode_json)
from .constants import MAX_REQUEST_BYTES, SUCCESS

FIXTURE_PATH = Path("docs/contracts/fixtures/command-observation-evidence-adapter-v1.json")
WRAPPER_FIELDS = {"api_version", "expected", "request"}
EXPECTED_FIELDS = {
    "canonical_command_json", "canonical_evidence_record_json",
    "canonical_observation_json", "canonical_request_json", "command_sha256",
    "evidence_record_sha256", "request_sha256", "result",
...
def computed_expected(request: dict[str, object]) -> dict[str, object]:
    ...
    return {
        "canonical_command_json": canonical_command(observation["command"]).decode("utf-8"),
        ...
        "command_sha256": compute_command_digest(request),
...
def _validate_golden_fixture(repo_root: Path) -> list[str]:
    ...
    try:
        raw = read_bounded_file(path, label="command observation evidence golden fixture",
    ...
    if not _exact_fields(value, WRAPPER_FIELDS, "golden", issues):
        return issues
    expected_api = "forgeos.governance.command-observation-evidence-adapter.fixture/v1"
