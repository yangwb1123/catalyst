#!/usr/bin/env python3
"""CLI for strict ADR-0052 production and fixture validation."""
...
if __package__ in {None, ""}:
    from evolve_locator_observation_producer import (  # type: ignore
        CHECKED, decode_production, validate_golden_fixture, validate_production,
    )
else:
    from . import CHECKED, decode_production, validate_golden_fixture, validate_production
from evolve_locator_observation_producer.constants import MAX_PRODUCTION_BYTES  # noqa: E402
...
def _production_issues(path: Path) -> list[str]:
    try:
        raw = read_bounded_file(
            path, label="local Evolve locator production",
