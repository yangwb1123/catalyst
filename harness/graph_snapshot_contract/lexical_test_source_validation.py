"""Fail-closed exact reconstruction for complete ADR-0066 envelopes."""
...
from .constants import ENVELOPE_FIELDS, REQUEST_FIELDS, SNAPSHOT_FIELDS
from .lexical_test_source_constants import (
    CANONICALIZATION, ENVELOPE_API, MAX_AGGREGATE_LOCATORS, MAX_CROSSWALKS,
    MAX_EDGE_UNION, MAX_ENVELOPE_BYTES, MAX_LOCATORS_PER_RECORD, MAX_NODES,
    MAX_REQUEST_BYTES, MAX_SNAPSHOT_BYTES, MAX_UNRESOLVED_EDGES,
    MAX_UNRESOLVED_NODES, REQUEST_API, SNAPSHOT_API, TEST_SOURCE_PROFILE_ID,
)
...
def _locator_limits(snapshot: dict[str, object]) -> None:
    total = 0
    for name in ("nodes", "edges", "unresolved_nodes", "unresolved_edges"):
        for index, record in enumerate(snapshot[name]):
            if not isinstance(record, dict):
                raise ContractError(f"snapshot.{name}[{index}]: expected object")
            locators = _bounded_list(
                record.get("source_locators"), MAX_LOCATORS_PER_RECORD,
                f"snapshot.{name}[{index}].source_locators")
            total += len(locators)
    if total > MAX_AGGREGATE_LOCATORS:
        raise ContractError("snapshot: aggregate source locator limit exceeded")


def _snapshot_limits(value: object):
    ...
    _bounded_list(
        coverage.get("surfaces"), 11, "snapshot.coverage.surfaces", exact=11)
...
def _unsupported_profile(value: object) -> bool:
    ...
    return (
        ...
        isinstance(request, dict) and (
            ...
            request["projector_profile_id"] != TEST_SOURCE_PROFILE_ID) or
        isinstance(snapshot, dict) and (
            ...
            snapshot["profile_id"] != TEST_SOURCE_PROFILE_ID))


def _validate(raw: bytes) -> None:
    broad = decode_profile_discriminators(
        raw, max_bytes=MAX_ENVELOPE_BYTES, label="ADR-0066 envelope")
    if _unsupported_profile(broad):
        raise ContractError(
            "unsupported_profile: unsupported test-source snapshot version or profile")
    ...
    expected = derive_test_source_envelope(
        graph_bytes, request["graph_observation_sha256"], request["run_id"],
        request["project_id"])
    if canonical_json(expected, max_bytes=MAX_ENVELOPE_BYTES) != raw:
        raise ContractError("envelope: not the unique exact ADR-0066 reconstructed value")


def validate_test_source_envelope_bytes(raw: bytes) -> list[str]:
    ...
    try:
        ...
        return ["ADR-0066 validation exhausted bounded memory"]
    except (KeyError, TypeError, ValueError, AttributeError, IndexError,
            UnicodeError, RecursionError) as error:
        return [f"ADR-0066 fail-closed nested value: {error}"]
