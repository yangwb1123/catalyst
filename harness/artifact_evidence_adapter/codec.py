"""Strict bounded adapter JSON codec with one legacy `_format` exception."""
...
def _valid_key(key: object, path: tuple[str, ...]) -> bool:
    if key == "_format" and path == ("request", "artifact"):
        return True
...
def _walk_limits(value: object, path: tuple[str, ...], depth: int = 1) -> None:
    if depth > MAX_DEPTH:
    ...
    if isinstance(value, list):
        ...
        for item in value:
            _walk_limits(item, path + ("[]",), depth + 1)
        return
    ...
    for key, item in value.items():
        if not _valid_key(key, path):
            raise ContractError(f"object key {key!r} is not ASCII snake_case")
        _walk_limits(key, path, depth + 1)
        _walk_limits(item, path + (key,), depth + 1)


def canonical_json(value: object, *, root: tuple[str, ...] = ("request",)) -> bytes:
    """Return adapter-canonical bytes without changing Governance canonical JSON."""
    try:
        _walk_limits(value, root)
...
def decode_json(raw: bytes, *, root: tuple[str, ...] = ("request",),
                max_bytes: int = MAX_REQUEST_BYTES) -> object:
    """Decode strict UTF-8 JSON while preserving the adapter key exception."""
    try:
...
def canonical_artifact(artifact: object) -> bytes:
    """Canonicalize the exact legacy Artifact v1 source object."""
    return canonical_json(artifact, root=("request", "artifact"))
