...
- The projection runs inside one deferred SQLite transaction, so its intended
  consistency boundary is one database snapshot. A dedicated concurrent
  terminalization regression remains required before a later effectful step
  relies on complete-before-or-after behavior.
...
The next effectful slice may execute at most one Core-selected node and must
require fresh consent for that exact provider request. Only after that boundary
is independently validated should a durable controller be added. Concurrent
wave execution requires a new schedule/progression version plus precise
per-task crash evidence; it is not an extension of this v1 contract.
...
SQLite-focused tests currently cover schedule-only projection, candidate plus
prepared-request identity, preservation of non-contiguous evidence, a missing
schedule and an out-of-schedule candidate row. Process-level CLI tests run the
repository-built official Core and wrong-pin failure path, compare logical Hub table
snapshots, preserve workspace sentinels, poison credential/endpoint inputs,
reject private-output leakage and observe no loopback connection. Exhaustive
32-node, concurrent terminalization and complete stored-corruption tests remain
follow-up evidence. These tests also do not prove confinement of an arbitrary
