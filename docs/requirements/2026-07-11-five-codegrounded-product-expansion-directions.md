...
`persist.Save` 有 `retain` 参数（保留历史 checkpoint 快照），但 trace/memory 是纯粹的追加日志——**没有文件名分片、没有非前缀、没有进程级隔离**。
...
// forge-core/cmd/forge/evolve.go:413  （trace 路径同理）
...
```go
// forge-core/internal/persist/checkpoint.go:92
...
// 使用 os.O_APPEND 单行追加
// 内核保证 < 4096 字节的 write 原子，但多进程各行交错
```

内核的 `O_APPEND` 保证单次 `write()` 原子写入（≤ PIPE_BUF），但**不保证**两个进程写入的行不发生交错：进程 A 写入 `{ "kind": "gap", "topic": "auth" }\n`，进程 B 在 A 的 write(2) 返回前也 write，结果文件中出现 `{ "kind": "gap"{ "kind": "finding"\n }。

（虽然单行 JSON + `\n` 大概率单次 write 成功，但这是**实现细节**——POSIX 不保证 ≥ 1 字节的 O_APPEND 原子性。）

**4. `forge status` 的 JSON 输出已有 `size_bytes`/`age` 字段，但缺少运行时进程身份**

```go
// forge-core/cmd/forge/validate.go:35-47
...
| 场景 | 当前行为 | 预期行为 |
|------|---------|---------|
| 两个 `forge run build` 并行执行 | trace/memory 文件内容交错+损坏 | 各有独立 run 子目录(`.forge/runs/<run-id>/`) |
...
```go
// forge-core/cmd/forge/detect.go:118
...
// TierFor 接收: agent 角色 + 当前 mode × lifecycle + (可选的)risk
// 但从不接收: language complexity, test maturity, CI presence
```

一个 Go 静态编译项目（高测试覆盖率 + CI 完善 + 二进制体积大）与一个 Python 脚本项目（JSON-only + 零测试），在目前的路由器中得到完全相同的 tier 分配——只要 mode×lifecycle 相同。但这些差异恰恰是模型选择的信号：Go 项目更需要 Opus 的架构 reasoning，Python 脚本的 CRUD 代码可能 sonnet 就足够了。

**2. Risk Classifier (`internal/risk`)**

```go
// forge-core/internal/risk/risk_diff.go
// FromChangedPaths 只读改动的文件路径 → 推风险特征
// 从不读 projectProfile 中的 lifecycle/HasTests/HasCI
```
...
|------|---------|---------------------------|
| Python 脚本项目（零测试）跑 `forge route` | tier 取决于 mode×lifecycle，与项目类型无关 | 从 detect 读取 `HasTests=false` → risk+1 档 → 自动升 tier |
...
1. **Detect Pipeline**: 将 `detectProject` 的输出注入 `routing.TierFor` 和 `risk.FromChangedPaths` 的**可选上下文参数**中——让 detect 信号成为路由/风险评分的可选的「加层」。
...
这不是大问题。但更关键的是：**没有 `quick`/`fuzz`/`cmp`** 等标准库以外的一切测试库——Go 1.23 标准库的 `testing/quick` 只有最基本的属性检查，`cmp` 不在其中。
...
Go 标准库的 `testing/f` 支持 fuzz，但 forge-core 的复杂嵌套状态（workflow JSON 解码 + mode 解析 + converge 评估）非常适合 fuzz 测试——目前零 fuzz 入口可见：

```bash
$ grep -rn "Fuzz" forge-core/ --include="*_test.go"
# 仅 routing/testdata/fuzz/ 下有 fuzz corpus 数据，没有 fuzz 测试函数
```
...
```go
// forge-core/internal/memory/memory.go:117
...
```go
// forge-core/internal/risk/risk_diff.go
// filepath.Walk / os.ReadDir 的返回顺序不保证一致
```

`risk.FromChangedPaths` 依赖文件路径扫描。虽然最终评分可能是顺序无关的，但**传播路径**可能间接影响确定性。
...
```go
// forge-core/internal/persist/checkpoint.go:52
...
- **方向一**（运行身份隔离）：并发安全和 `.forge/` 数据完整性已在 `strategic-extensions-v22-v33.md` 系列中被提到（作为「运行身份」和「会话隔离」的子问题），但从未被作为独立方向拆开分析。
- **方向二**（detect 回灌）：`forge detect` 曾在 `expansion-production-readiness.md` 和 `strategic-expansion-five-novel.md` 中被提到，但这是关于 detect 的 CLI 功能完整性，而非其与 routing/risk/mode 的集成缺口。
- **方向三**（结构化输出）：`genuine-uncovered-five-binary-state-output-session-datalifecycle.md` 的方向三「统一结构化输出协议」覆盖了 17 个子命令 `--json` 的**有无**；本文补充延伸至**跨子命令 schema 一致性**和**与 CI/CD/IDE 集成**。
