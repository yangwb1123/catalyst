...
```go
// forge-core/internal/orchestrator/parallel.go:153-169
go func(i int) {
    defer wg.Done()
...
```go
// forge-core/internal/orchestrator/parallel.go:17-21
...
```go
// forge-core/internal/orchestrator/orchestrator.go:242-254
// runAgentPhase → exec.Execute → 返回 nil → 立即下一步
...
| `genuinely-uncovered-five-deep-runtime-gaps.md`「产出审计」 | 关注审计和合规视角。本文关注运行时编排的正确性——在推进到下一 phase 之前验证声明 |
...
```go
// forge-core/internal/orchestrator/parallel.go:95-105
...
```go
// forge-core/internal/orchestrator/loop.go:208-239
...
```go
// forge-core/internal/orchestrator/mode_gating.go:65-88
...
```go
// forge-core/internal/converge/converge.go:66-70
