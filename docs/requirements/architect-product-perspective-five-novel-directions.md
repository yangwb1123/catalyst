...
如果 claude 在 stderr 上输出 `[progess: 50%]` 或 `[warning: rate limit approaching]`,而 stdout 上有 JSON:
```
[progess: 50%]{"type":"result","result":"done","total_cost_usd":0.054}
```
...
| 5 | 上下文感知恢复 | P1 | 可靠性 | ⭐⭐⭐⭐ | 零 | 1 sprint | bob 预算浪费,更好的退化行为 |
...
3. **环境侧信道防护(方向四)**:这是最低成本(A 只需修改一个 `childEnv` 函数)但最高杠杆的改进——目前每个 agent 子进程都能读取父进程的全部环境,包括 API key 和其他敏感凭据。
