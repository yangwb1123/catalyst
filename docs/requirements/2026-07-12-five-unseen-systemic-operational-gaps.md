...
> **代码证据**: `forge-core/cmd/forge/main.go:242` · `forge-core/internal/orchestrator/executor.go:28-40` ·  
>   `forge-core/internal/memory/memory.go:185` · `forge-core/internal/trace/trace.go:116` ·  
...
```go
// forge-core/cmd/forge/main.go:242
fs.StringVar(&o.executor, "executor", "dry", "agent executor: dry|command")
```

When `--executor dry` (or default), the orchestrator instantiates `DryRunExecutor` which only logs and returns nil.

**证据 B: DryRunExecutor.Execute 只打一行叙述**

```go
// forge-core/internal/orchestrator/executor.go:28-40
type DryRunExecutor struct {
    Log  func(format string, args ...any)
}

func (d DryRunExecutor) Execute(_ context.Context, p asset.Phase, mode string) error {
    // Log the routing decision; no LLM invoked.
    d.logf("implements %s (%s)", p.Name, mode)
    return nil
}
```
...
- `memory.go:185` `Append` ← 不被执行（无 agent phase → 无 findings）
- `trace.go:116` `Emit` ← 不被调用（Span 的 defer 不触发）
- `converge.go` ← 不被触发（roadmap 不变 → converge 不 MET）

**证据 D: `forge evolve` 的 LoopEngine 在 dry-run 下永不迭代**
...
> **代码证据**: `forge-core/internal/routing/routing.go:297-310` `BudgetAdjustTier` ·  
>   `forge-core/internal/orchestrator/backoff.go` · `forge-core/cmd/forge/cost.go` `runBudget`  
...
```go
// forge-core/internal/routing/routing.go:297-310
...
- **reviewer 本身也被降级**：`opusFloorAgents` 保护 reviewer 不被降级，但 *implementer*（被降级者）的低质量输出增加了 reviewer 的工作量。reviewer 每次 REVIEW 都要付费（且被 loop-back 多次调用），反而消耗更多预算。
- **`DowngradeOne` 从 haiku 再降级会怎样**：`DowngradeOne("haiku")` → `default: return Haiku`（见 routing.go:~330）。所以 haiku 不会进一步降级——但这也意味着 haiku 是最终停留的低档，其质量差异最大。
- **预算耗尽与降级同时发生**：`spendRatio >= 1.00 && critical` → `EscalateToHuman`。但非 critical task 在 `spendRatio >= 1.00` 时直接 collapse 到 task_type floor（`routing.go:~275`）。如果 floor 也是 haiku（如 `"docs"`），降级+collapse 叠加。
...
> **代码证据**: `forge-core/internal/orchestrator/waves.go` ·  
>   `forge-core/internal/orchestrator/parallel.go:90-120` `runWave` ·  
>   `forge-core/internal/orchestrator/backoff.go:53-80` `overloadBackoff`（无抖动）  
...
```go
// forge-core/internal/orchestrator/backoff.go:53-80
const (
...
// 注释原文:
// "v1 single-run: NO JITTER — jitter only matters once many agents retry in
//  parallel and could thunder-herd the backend in lockstep"
func overloadBackoff(attempt int) time.Duration {
    ...
    d := overloadBackoffBase << attempt // pure deterministic shift
...
```go
// forge-core/internal/orchestrator/parallel.go:90-120
...
// 一个 0-dep 的 20-phase workflow 产生一个 20-phase 的单波
// Waves 不限制每波 phase 数
```
...
> **优先级**: 🔴 **P1** | **类别**: 安全 · 合规 | **风险**: 凭据泄漏到 LLM 进程  
> **代码证据**: `forge-core/internal/orchestrator/command_executor.go:293-301` `childEnv`  
...
```go
// forge-core/internal/orchestrator/command_executor.go:293-301
...
> **代码证据**: `forge-core/internal/trace/trace.go`（trace.jsonl）· `forge-core/internal/memory/memory.go`（memory.jsonl）·  
>   `forge-core/internal/routing/scorecard.go`（scorecards.json）· `forge-core/internal/persist/checkpoint.go`（checkpoint.json）·  
>   `forge-core/internal/doctor/doctor.go`（无跨文件一致性检查）  
...
| `.forge/scorecards.json` | atomic rewrite | `scorecard.LoadScorecards` | 路由质量历史 |
...
`persist.Save` 保存 `SpentUsdMicros`（从 cost.go 读取），但不读取或写入 scorecard 数据。`scorecard.LoadScorecards` 独立读写 scorecards.json，不与 checkpoint 同步。

**证据 D: doctor.go 无跨文件一致性检查**

`doctor.go` 的 `checkpointCheck`、`traceCheck`、`memoryCheck` 各自独立检查单一文件，从不验证一个文件中的数据是否与另一个文件一致。

### 产品影响

- **`forge doctor` 不检查存储一致性**：验证通过代码审计（`doctor.go` 只检查单文件可读性、非空性、最后一行完整性，不做交叉校验）。
...
| # | 方向 | 关键文件 | 关键行号 | 验证状态 |
|---|------|---------|---------|---------|
| 1 | 默认 dry-run | `cmd/forge/main.go` | L242 (`--executor dry` 默认值) | ✅ 确认 |
| 1 | DryRunExecutor | `internal/orchestrator/executor.go` | L28-40 (只记录不执行) | ✅ 确认 |
| 1 | 学习循环路径 | `trace.go:116`, `memory.go:185`, `converge.go` | 所有 Emit/Append/Converge | ✅ 确认 |
| 2 | BudgetAdjustTier | `internal/routing/routing.go` | L297-310 (无 DecisionEvent) | ✅ 确认 |
| 2 | 无断路器 | 全仓 grep `circuit.*breaker` | 零结果 | ✅ 确认 |
| 3 | overloadBackoff 无抖动 | `internal/orchestrator/backoff.go` | L53-80 (注释写明 NO JITTER) | ✅ 确认 |
| 3 | runWave | `internal/orchestrator/parallel.go` | L90-120 (全 goroutine 并行) | ✅ 确认 |
| 3 | Waves 无大小限制 | `internal/orchestrator/waves.go` | 全文件 (无 max 参数) | ✅ 确认 |
| 4 | childEnv 只过滤一个变量 | `internal/orchestrator/command_executor.go` | L293-301 (仅 FORGE_AGENT_DEPTH) | ✅ 确认 |
| 5 | 无跨存储一致性 | `internal/doctor/doctor.go` | 全文件 (独立检查各文件) | ✅ 确认 |
| 5 | 四个存储独立写入 | trace/memory/scorecard/checkpoint | 各自有独立写入者 | ✅ 确认 |
