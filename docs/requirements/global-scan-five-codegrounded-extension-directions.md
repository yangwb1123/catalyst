...
>   `.agent/` 完整治理骨架（5 workflow + 12 agent 卡 + 9 skill 卡）· `ai-dev/` · `examples/` ·
>   `pi-batch.py` · `.github/workflows/forge.yml`）、
...
3. **trace Event 加 `mode_snapshot` 可选字段**——`omitempty` 向后兼容，记录该事件发生时的 `{mode, lifecycle, gate_set}`。run 开始时固定——通过 `buildRunEngine` 注入。这样 trace.jsonl 既可审计（现在已可）又可回放（新增能力）。
...
`.agent/` 完整骨架（5 workflow + 12 agent 卡 + 9 skill 卡）· `ai-dev/` · `examples/` ·
