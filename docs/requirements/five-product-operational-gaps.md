...
作为一个**被团队采用、被运维、被长期的系统**时的五类缺失基础设施。
...
1. **checkpoint 损坏 = 死路** — Load 遇到损坏的 JSON 返回错误，调用者（cmd/forge）直接退出:
   ...
   func Load(path string) (*Checkpoint, error) {
       ...
       if err != nil { /* 文件缺失: 返回 nil, nil —— 这是冷启动的正常情况 */ }
       var cp Checkpoint
       if err := json.Unmarshal(data, &cp); err != nil {
           return nil, fmt.Errorf("persist: load checkpoint: %w", err) // 损坏 = 硬错误
       }
   }
   ```
   调用方（`resumeStart`）面对错误直接 abort，没有任何尝试从 `.bak` 备份恢复。

2. **无 checkpoint 备份** — `persist.Save` 使用 temp+rename 保证原子性（防止写入中 crash），
   但**不保留旧版本**。一旦新 checkpoint 写入成功覆盖旧版，旧版永久丢失:
   ```go
   // forge-core/internal/persist/checkpoint.go:128-134
   func Save(path string, cp Checkpoint) error {
       tmp := path + ".tmp"
       writeSynced(tmp, data)   // 写入 temp
       os.Rename(tmp, path)     // 原子覆盖
       // 旧 checkpoint 永久丢失
   }
   ```

3. **memory 损坏 = 全部丢失** — Load 遇到损坏条目返回错误，且没有索引重建或跳过损坏条目的选项:
   ```go
   ...
   func Load(path string) ([]Entry, error) {
       ...
       for sc.Scan() {
           ...
           if err := json.Unmarshal([]byte(sc.Text()), &e); err != nil {
               return nil, fmt.Errorf("memory: …") // 一个坏行 → 全文件废弃
           }
       }
   }
   ```
...
4. **trace 损坏 = scorecard 不可用** — `scorecard_wind.go` 在整个文件上做 `sc.Scan()`，
...
5. **无退化模式 flag** — forge-core 没有 `--repair`、`--force`、`--recover-from-backup` 等
