#!/usr/bin/env python3
"""Evaluate one exact CapabilityGrant declared-assessment request."""

import sys

if not sys.flags.isolated or not sys.flags.dont_write_bytecode:
    sys.stderr.write(
        "capability-grant assessment rejected: isolated no-bytecode Python (-I -B) is required\n"
    )
    raise SystemExit(1)
...
USAGE = "assess_declared_capability_grant.py < canonical-assessment-request.json"
CONTRACT_MODULE = "_forgeos_bundled_capability_grant_contract_v1"
...
def _load_contract() -> ModuleType:
    ...
    package = Path(__file__).resolve().parent / "_vendor/capability_grant_contract"
...
def _read_request() -> bytes:
    ...
    while True:
        ...
        if len(raw) > MAX_REQUEST_BYTES:
            raise AdapterError("assessment request exceeds its byte bound")


def assess(raw: bytes) -> bytes:
    """Return the exact authority-neutral assessment payload without framing LF."""
    contract = _load_contract()
    request = contract.decode_request(raw)
    assessment = contract.evaluate_declared_assessment(request)
    return contract.canonical_json(assessment)
...
def main(argv: list[str]) -> int:
    if argv:
        ...
        return 2
    try:
        output = assess(_read_request()) + b"\n"
        _write_all(output)
    except Exception:
        sys.stderr.write("capability-grant assessment rejected\n")
        return 1
    return 0
