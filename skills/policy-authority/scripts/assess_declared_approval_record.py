#!/usr/bin/env python3
"""Evaluate one exact ApprovalRecord declared-assessment request."""

import sys

if not sys.flags.isolated or not sys.flags.dont_write_bytecode:
    sys.stderr.write(
        "approval-record assessment rejected: isolated no-bytecode Python (-I -B) is required\n"
    )
    raise SystemExit(1)
...
USAGE = "assess_declared_approval_record.py < canonical-assessment-request.json"
CONTRACT_MODULE = "_forgeos_bundled_approval_record_contract_v1"
...
def _load_contract() -> ModuleType:
    ...
    package = Path(__file__).resolve().parent / "_vendor/approval_record_contract"
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        sys.stderr.write("approval-record assessment rejected\n")
        return 1
    return 0
