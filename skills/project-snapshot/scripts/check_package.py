#!/usr/bin/env python3
"""Validate the closed project-snapshot portable package."""

import sys

if not sys.flags.isolated:
    sys.stderr.write(
        "project-snapshot package rejected: isolated Python startup required; "
...
def _validate_manifest_root(manifest: dict[str, object]) -> list[tuple[str, int, int, str]]:
    ...
    if (manifest["api_version"] != API_VERSION or manifest["manifest_path"] != MANIFEST or
            manifest["package_name"] != "project-snapshot"):
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print(f"project-snapshot package rejected: {error}", file=sys.stderr)
        return 1
    print("project-snapshot portable package VALID")
    return 0
