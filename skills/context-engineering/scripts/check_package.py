#!/usr/bin/env python3
"""Validate the closed context-engineering portable package."""

import sys

if not sys.flags.isolated:
    sys.stderr.write(
        "context-engineering package rejected: isolated Python (-I) is required\n"
    )
    raise SystemExit(1)
...
def _validate_manifest_root(manifest: dict[str, object]) -> list[tuple[str, int, int, str]]:
    ...
    if (manifest["api_version"] != API_VERSION or manifest["manifest_path"] != MANIFEST or
            manifest["package_name"] != "context-engineering"):
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print(f"context-engineering package rejected: {error}", file=sys.stderr)
        return 1
    print("context-engineering portable package VALID")
    return 0
