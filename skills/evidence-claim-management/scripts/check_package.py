#!/usr/bin/env python3
"""Validate the closed evidence-claim-management portable package."""

import sys

if not sys.flags.isolated:
    sys.stderr.write(
        "evidence-claim-management package rejected: isolated Python (-I) is required\n"
    )
    raise SystemExit(1)
...
def _validate_manifest_root(manifest: dict[str, object]) -> list[tuple[str, int, int, str]]:
    ...
    if (manifest["api_version"] != API_VERSION or manifest["manifest_path"] != MANIFEST or
            manifest["package_name"] != "evidence-claim-management"):
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print(f"evidence-claim-management package rejected: {error}", file=sys.stderr)
        return 1
    print("evidence-claim-management portable package VALID")
    return 0
