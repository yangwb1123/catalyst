#!/usr/bin/env python3
"""Validate the closed knowledge-graph-curation portable package."""

import sys

if not sys.flags.isolated or not sys.flags.dont_write_bytecode:
    sys.stderr.write(
        "knowledge-graph-curation package rejected: isolated no-bytecode Python (-I -B) is required\n"
    )
    raise SystemExit(1)
...
def _validate_manifest_root(manifest: dict[str, object]) -> list[tuple[str, int, int, str]]:
    ...
    if (manifest["api_version"] != API_VERSION or manifest["manifest_path"] != MANIFEST or
            manifest["package_name"] != "knowledge-graph-curation"):
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print(f"knowledge-graph-curation package rejected: {error}", file=sys.stderr)
        return 1
    print("knowledge-graph-curation portable package VALID")
    return 0
