#!/usr/bin/env python3
"""Project the exact ADR-0066 Go lexical test-source request wire."""

import sys

if not sys.flags.isolated or not sys.flags.dont_write_bytecode:
    sys.stderr.write(
        "Go test-source GraphSnapshot projection rejected: isolated no-bytecode Python (-I -B) is required\n"
    )
    raise SystemExit(1)
...
try:
    ...
    sys.stderr.write("Go test-source GraphSnapshot projection rejected\n")
    raise SystemExit(1)

if __name__ == "__main__":
    raise SystemExit(_adapter.main(_adapter.GO_TEST_SOURCE, sys.argv[1:]))
