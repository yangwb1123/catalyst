"""Deterministic exact ADR-0053 bytes to partial GraphSnapshot projection."""
...
from .constants import (
    CANONICALIZATION, ENVELOPE_API, ENVELOPE_DOMAIN, MAX_ENVELOPE_BYTES,
    GRAPH_API, GRAPH_PROFILE, MAX_GRAPH_BYTES, MAX_REQUEST_BYTES,
    MAX_SNAPSHOT_BYTES, PROFILE_ID, REQUEST_API, REQUEST_DOMAIN,
)
from .profiles import validate_hash, validate_identifier
from .provenance import extractor_record, source_record
from .snapshot import build_snapshot
from .topology import (
    build_crosswalk, build_edges, build_nodes, enforce_locator_aggregate,
)
...
def _request(graph_bytes: bytes, graph_sha256: object, run_id: object,
    ...
    if not isinstance(graph_bytes, bytes) or len(graph_bytes) > MAX_GRAPH_BYTES:
        raise ContractError("ADR-0065 graph observation must be bounded bytes")
    _reject_unsupported_graph_profile(graph_bytes)
    graph = validate_graph_bytes(graph_bytes)
    declared_graph_sha = validate_hash(
        graph_sha256, "request.graph_observation_sha256")
    if observation_digest(graph) != declared_graph_sha:
    ...
    request = {
        ...
        "graph_observation_sha256": declared_graph_sha,
        "project_id": checked_project, "projector_profile_id": PROFILE_ID,
...
def _projection(graph: dict[str, object], graph_sha256: str,
                project_id: str, request_sha256: str):
    source = source_record(graph, graph_sha256)
    extractor = extractor_record(source)
    nodes, module, by_package, locator_map, files = build_nodes(
        graph, project_id, source, extractor)
    edges = build_edges(graph, module, by_package, locator_map, files,
                        source, extractor)
...
def derive_envelope(graph_json: bytes, graph_sha256: object, run_id: object,
                    project_id: object) -> dict[str, object]:
    """Build the unique envelope using only caller-supplied bounded bytes."""
