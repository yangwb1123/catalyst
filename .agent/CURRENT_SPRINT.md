...
- **Graph 下一 effectful slice**:从完整重验的 v3 Run/contract/seq-3 head/exact prepared bytes 导出 private release-control snapshot，由 Go core 独立生成绑定同一 request/head 的 content-addressed authorization；最终 invocation 才取得 fresh off-machine consent、读取 credential 并复验 destination/model/pricing/budget。SQLite 必须在同一次 invocation 以全局 project-lane 排他和 seq-3/head CAS 原子 claim，只向单赢家交出不可复制的 exact-byte authority，并立即进入 `dispatch_unknown`；timeout/cancel/EOF/lease 均不得自动重发。随后还需 node-specific result adapter、bounded artifact/terminal receipt，以及由 Go core 验证 receipt 后才允许 successor advance；这些均未被 Sprint 50 的 prepared-request presence 冒充完成。
