...
- **Graph 下一 effectful slice**:在任何 provider request 前重新验证 contract/source/seq-head/project lane/destination/pricing，要求 fresh off-machine consent，原子追加 dispatch-release event 并只向单赢家交出不可复制的 exact-byte authority；claim 后立刻视为 `dispatch_unknown` 且绝不因 timeout/cancel/EOF/lease 自动重发。随后还需 node-specific AgentRuntime adapter、真实 OS/workspace isolation、bounded result artifact/terminal receipt，以及由 Go core 验证 receipt 后才允许 successor advance；这些均未被 Sprint 48 的 contract presence 冒充完成。
