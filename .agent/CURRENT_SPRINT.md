...
- **Graph 下一 effectful slice**:先定义 exact registered-destination provider factory、immutable pricing artifact/cost algorithm、node result 与 Core-validated terminal receipt。最终 invocation 才取得 fresh off-machine consent、读取 header-safe credential，并复验 authorization/destination/model/pricing/budget；SQLite 必须在同一次 invocation 以全局 project-lane 排他和 seq-3/head CAS 原子 claim，只向单赢家交出不可复制的 exact-byte authority并立即进入 `dispatch_unknown`。claim/send 不能作为孤立 API 落地：同一完整生命周期还必须 bounded 收集 terminal/EOF、持久化 result/receipt、释放 lane、传播失败并 advance successor/wave；timeout/cancel/EOF/lease 均不得自动重发。
