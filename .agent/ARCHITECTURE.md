...
> **v2 现状**:forge-core Go 运行时当前 **29 包**(纯标准库零依赖),已落地 5 个核心引擎与可工作的本地 Agent-Runtime 切片:
