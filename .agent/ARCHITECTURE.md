...
ADR-0106 Proposed candidate 增加唯一 reviewed consumer `internal/reconcile/application`：一次 pure `Decide` 对完整
caller-owned snapshot 重新验证 Domain、supplied-current identifier、assessment version/declaration/evidence 与 predecessor
progress，再确定性返回一个 passive pre-effect Decision。Policy/Approval/Budget status 固定为 authority-neutral declaration；
`ReadyWorkItem` 不请求状态迁移或释放执行 authority。该包无 I/O、store、journal/outbox、controller、Attempt、Runtime/
Harness Receipt、completion、canonical wire 或用户入口；effectful FC-07/08、F6/F7 与 Objective-to-Outcome 仍开放。

ADR-0107 Proposed 边界内已实现的 R0-C5 只关闭 FR-03a pure `AttemptRequest` construction boundary：
caller-supplied input 必须绑定完整 Platform Core Scope、exact Attempt/WorkItem/Project/ProjectSnapshot refs、正数 bounded
Objective/Change/WorkGraph/WorkItem versions、executor declaration、可选 snapshot-bound context Artifact 与 typed
workspace-capability/Grant、normalized typed Approval/effect sets、Attempt budget、timeout 和 Platform Core idempotency
key。成功对象防御性复制输入、字段 private、只读 getter，state 由构造器固定为 `requested`。它不认证任何
...
> **v2 现状**:在 `forge-core/` 执行 `go list ./...` 当前实测 **97 包**（94 个 `internal`，含嵌套包与仅测试使用的 `internal/adr`；另有 `cmd/forge`、独立非 Agent `cmd/forge-kernel` 和 R0 `cmd/forge-server`）。排除 `internal/adr` 后的产品口径为 **96 包**；除 `internal/controlstore` 固定的 `modernc.org/sqlite v1.57.0` 及其 `go.sum` 闭包外，其余现有 Go 边界保持原依赖模型。已落地 5 个核心引擎与可工作的本地 Agent-Runtime 切片:
...
>
> Sprint 136 以 ADR-0096 Proposed 候选补上 Scheduled Graph Progress Snapshot + Core Reconcile v1。Rust 在现有 exact-current v28 Hub 的单个 deferred read transaction 中完整重验 Run/Graph/schedule 与每 ordinal 的 candidate/request/lifecycle/terminal identity，只投影 content-free canonical snapshot；operator-pinned、SHA-256-bound Go Core 以 schedule-v1 serial policy 返回 `ready|claimed_unknown|manual_recovery_required|failed|failed_uncertain|completed|incompatible_progress`，Rust 严格重验 exact decision、digest 与 source binding。`ready` 只携带 exact next ordinal/node observation；命令不读取 credential/workspace、不构造 provider、不联网、不 claim lane、不写 Hub，也不执行、恢复、重试、重发、adjudicate 或授予 successor authority。Effectful one-node step 和 durable whole-Graph controller 继续开放。
>
> Sprint 137 闭合 ADR-0096 的 pre-effect storage validation：真实 scheduled claim→terminalize 与已固定 deferred reader 交错，只能看到完整前态，fresh reader 看到完整 completed/receipt 后态；真实 SQLite 覆盖 32-node count/order，Core 覆盖 31-completed→ordinal-31-ready 及 32-completed，并确认这些 representative canonical inputs 均在 64 KiB 内；42 个独立 candidate/request/lifecycle/orphan/count/terminal corruption 全部 fail closed。该证据不宣称构造了 byte-maximal identifiers 或 ready node。正向 fixture 同时修复了 scheduled claim `INSERT_SQL` 未代入 table 常量的生产缺陷。审计确认现有 Go scheduled-release v1 固定 initial/ordinal 0，故下一步先做绑定 exact snapshot+reconcile decision 的 zero-effect successor-capable release v2，再做 effectful one-node step，controller 继续更晚。
>
> 上述 reconcile no-effect 边界只适用于 Forge Runtime 合同与已测试的 repository-built official Core 路径。Core 是 operator-trusted same-user TCB；digest pin 只证明执行字节，handshake 只证明协议兼容。空环境、sealed bytes、bounded I/O 不是 Core sandbox，Runtime 未实施 filesystem/network/syscall confinement 或 publisher/function attestation。CLI JSON 以 `effect_facts_scope="forge_runtime"` 约束独立的 `runtime_effect_facts`，并在 `core_trust_boundary` 中把 same-user/operator-trust/identity/handshake/empty-env 报为 true，把 filesystem/network isolation、effect containment 与 effect attestation 报为 false；Human 输出也重复 TCB 警示。这些字段是合同与缺口披露，不得解释为 arbitrary pinned child 的 syscall observation。
>
> Sprint 138 以 ADR-0097 Proposed 候选交付独立 Scheduled Ready-Node Release Authorization v2。Runtime 先从 atomic progress S0 调用既有 pinned reconcile Core 并验证 exact `ready` decision，再以 S0 digest 与 selection 在 deferred transaction A 中完整重验 progress、selected initial/successor candidate、prepared request、有序直接前驱 receipts 与可选首直接前驱 result artifact；A 关闭后才把 decision 绑定进 control 并调用 handshake-2 authorization Core，后者重跑 exact reconcile 和 selected source rebuild。第二个 atomic bundle B 必须与 A source 及用同一 decision 构造的 control exact 相等，Rust 才接受 metadata-only `maximum_future_node_releases=1` authorization。compiled cross-language 覆盖 initial、successor 与完整 ordinal 31/31 receipts；真实 Application + SQLite 交错在 A 后暂停 authorization Core port，legal claim 可无锁提交，B 随后拒绝 stale source，较低层 snapshot 测试另覆盖 terminalization。两个 Core child 均不跨 transaction。ADR-0033 的 predecessor output 继续接受任意 nonempty valid UTF-8；6,553,926-byte Prompt ceiling 容纳 1 MiB control-scalar worst escaping，Go 对 U+2028/U+2029 采用 Rust 既有 raw-scalar canonical spelling，整份 content control 已跨语言验证，task/acceptance prose grammar 不变。该命令无 migration/Hub logical write/credential/workspace/provider/network/consent/lane/send/result/advance effect，且 B 后仍可 stale；effectful one-node step 必须另做 snapshot-to-claim CAS、fresh 双 consent 与单赢家 no-resend lifecycle。
>
> Sprint 139 以 ADR-0098 Proposed 候选交付公开 effectful scheduled one-node step。`group graph run step` 以 caller 预期的 request ID/ready-authorization digest、operator-pinned Core、exact pricing、fresh off-machine consent 与条件式 predecessor-content consent 重新运行 A/Core/B；provider 构造后仍保持 unopened，直到 SQLite `BEGIN IMMEDIATE` 把 current ready source、exact v2 release/request 与跨 legacy/ready family 的 Hub-global Project lane 一起 CAS，commit 后才返回 non-`Clone` authority。赢家至多 poll 一次 bounded provider stream，terminal 或 artifact-only quarantine 释放 lane；任何 claimed/quarantined/adjudicated/terminalized re-entry 都不重发。调用级 effect receipt 区分入口 replay、做过 preclaim 的 CAS loser、terminal winner 与持久 quarantine；protocol handshake 与 stored terminal receipt 分开披露，claim/post-claim 不确定错误也固定输出不重发事实。Linux owner sidecar 以 request+random lane owner hash 路径、`0700` directory、`0600` create-new file、machine/boot/PID-namespace/time-namespace/PID/process-start 与 device/inode 绑定；machine 不同失败关闭，同 machine 的旧 boot 判 dead，同 boot 才在读 `/proc/<pid>/stat` 前要求两个 namespace exact 相等且 `/proc/self` numeric target 精确等于 `getpid()`。directory advisory lock 把任意条目总数硬限为 1024 且不自动 scavenging。hard crash/claim or terminal commit uncertainty 保留 owner，显式 adjudication 只接受 durable exact owner 且 `dead|pid_reused`，并在 guarded immediate update commit 后清理；成功 adjudication 诚实报告 dispatch=false/database-write=true。commit 后 cleanup failure 不抹掉 lifecycle/effect receipt，而以 cleanup failed + presence unknown 单独披露。CLI 默认 metadata-only，`--include-result` 才揭示结果，并明示远端是否观测到 request 未被本地证明；每次调用至多一个 node，不 materialize/循环第二节点。该本地 single-consumption 不是 remote exactly-once，Core 仍是无 filesystem/network confinement 的 same-user TCB，exact-owner sidecar 也不是分布式身份或 fencing token；非 Linux effect/adjudication 明确失败为 unsupported，但其余 CLI 仍可编译。
>
...
ADR-0092/0093 在 Registry v39 只登记已完成交付的 Decision Capsule structural replay repository slice：四对象 DAG 对 caller-supplied ADR-0090 closure 做 pure validate/reseal/complete-manifest/compare；专用的后挂载 ReflectionReport refs 仅位于 outer closure 且保持 unresolved，上游 ArtifactRefs 则保持 opaque/uninterpreted。Scope digest 不变；exact19 仅分发 Python，Catalyst exact15 Go、exact14 Rust 和共享 registration 均不分发。32 项 attestations 与两项 replay controls 全 false；七项 completion 中仅窄 repository-slice completion 在正式验收后为 true，其余六项 broader completion claims 全 false。无 Skill、route、runtime、model/rule/world-state/history/Reflection consumer、persistence、PDP/controller；独立复核、fresh/legacy scaffold 与正式 `forge accept` 已通过（9 PASS、0 FAIL、2 个诚实 N/A）。两份 ADR 始终 Proposed/null；ADR-0038 仍为 ADOPTED-PARTIAL，完整 DecisionCapsule 与 AuthorizedTransactionSpec 保持开放。
