...
- [x] **Strict Hub schema ownership validation(ADR 0012，ADR 0013/0015/0017/0018/0019 延伸)**:每个 v0–v10 声明版本在任何迁移 DDL 前先按不可变 DDL 复验（v0 必须空 catalog），迁移提交前再复验完整 v10 的 23 张表、18 个显式索引、41 个隐式 autoindex exact/structural 契约与独占 main catalog。旧 CHECK/PK/UQ/FK/index 漂移、额外 table/view/trigger/virtual shadow 均失败关闭且不自动修复，迁移末端失败整链回滚。
