...
- [x] **Local-first Conversation Hub(ADR 0007)**:CLI 无路径进入 Global、有路径/`-C` 进入 Project、`--group` 进入 Group；SQLite 跨进程持久化 Project/Conversation/Prompt 与 frontend/backend/SSO 等带角色标签的联动。`group context` 现能在单一 SQLite 快照中生成有界、因果有序、带来源和 SHA-256 摘要的 Group/member-Project Prompt dossier，默认不显示正文且绝不读取项目文件；它仍只是本地预览，不会隐式外发或授予 capability。Prompt 仍由调用者显式追加；Project Run 现会以所选 user Prompt 为边界注入严格 512 KiB 的既有 user/assistant 历史。远程账号/同步、持久化 Group-context Run、共享 ACL 与多 Agent 组执行继续分期。
