...
- **O4 enforce**:**已切 `block`(已结)** —— O3 切片(url-shortener)跑通后,`harness/policies.yml` 已置 `enforce: block`,整仓 block 模式全绿。注:函数行数 / 循环依赖**已由 arch-check 实际执法**(`policies.yml` 标 ENFORCED,8 检查含 function-length/circular),全部红线均已机器执法;adapter 外部-linter 路径仍 v0.1 TODO,但 arch-check 已覆盖该需求,冗余。
