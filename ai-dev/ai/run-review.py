...
def _build_context(args) -> dict:
    """Load the context YAML (if any) and layer CLI overrides on top."""
    ctx = {}
    if args.context:
...
def _select_stages(args) -> list[str]:
    """Resolve --all / --stage into the ordered list of stage keys to run."""
    if args.all:
...
def main() -> None:
    ...
    stages_to_run = _select_stages(args)
