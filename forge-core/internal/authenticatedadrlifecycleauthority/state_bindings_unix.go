...
func (s *unixState) verifyBindings() error {
	if s == nil || s.lock == nil {
		return fmt.Errorf("lifecycle state is closed")
	}
...
func (s *unixState) verifyAuthority() error {
	if err := verifyAbsoluteDirectoryPath(s.authorityPath, s.authorityInfo, "authority root"); err != nil {
		return err
	}
	...
	if err != nil || openErr != nil || dirErr != nil || current.Mode()&os.ModeSymlink != 0 ||
		!os.SameFile(s.authorityInfo, current) || !os.SameFile(current, opened) || !os.SameFile(opened, dirInfo) {
		return fmt.Errorf("authority root binding changed")
	}
	return requireOwnedDirectory(opened, privateDir, "authority root")
}

func (s *unixState) verifyRepository() error {
	if err := verifyDirectoryBinding(s.repository); err != nil {
		return fmt.Errorf("repository binding changed: %w", err)
	}
...
func (s *unixState) verifyState() error {
	...
	if openErr != nil || dirErr != nil || !os.SameFile(s.stateInfo, current) ||
		...
		return fmt.Errorf("lifecycle state directory changed")
	}
	return requireOwnedDirectory(opened, privateDir, "lifecycle state directory")
}

func (s *unixState) verifyLock() error {
	opened, err := validateOpenLeaf(s.lock, privateMode, "lifecycle lock")
	if err != nil || !os.SameFile(s.lockInfo, opened) {
		return fmt.Errorf("lifecycle lock binding changed")
	}
	return verifyNamedIdentity(s.state, lockFile, s.lockInfo, privateMode)
}
