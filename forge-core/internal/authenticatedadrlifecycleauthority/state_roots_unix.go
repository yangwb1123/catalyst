...
func bindRepository(source string) (*directoryBinding, error) {
	...
	if err != nil || inspected == nil || !inspected.IsDir() {
		return nil, fmt.Errorf("repository root has unsafe ancestry")
	}
	...
	if err != nil || filepath.Clean(rechecked) != binding.path {
		...
		return nil, fmt.Errorf("repository root changed")
	}
...
func bindStableDirectory(value string) (*directoryBinding, error) {
	...
	if err != nil || before.Mode()&os.ModeSymlink != 0 || !before.IsDir() {
		return nil, fmt.Errorf("path must name real directory")
	}
	file, err := os.OpenFile(value, os.O_RDONLY|syscall.O_NONBLOCK|syscall.O_DIRECTORY|syscall.O_NOFOLLOW, 0)
	if err != nil {
		return nil, err
	}
...
func verifyDirectoryBinding(binding *directoryBinding) error {
	if binding == nil || binding.file == nil || binding.info == nil {
		return fmt.Errorf("directory binding unavailable")
	}
	if err := verifyAbsoluteDirectoryPath(binding.source, binding.info, "directory binding"); err != nil {
		return err
	}
...
func verifyAbsoluteDirectoryPath(value string, expected fs.FileInfo, label string) error {
	...
	if err != nil || filepath.Clean(resolved) != value {
		return fmt.Errorf("%s acquired symlink", label)
	}
	return nil
}
