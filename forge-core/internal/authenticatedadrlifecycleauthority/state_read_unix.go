...
func (s *unixState) current() (stateSnapshot, error) {
	...
	data, present, identity, err := readStateLeaf(s.state, stateFile, s.maxBytes)
	if err != nil {
		return stateSnapshot{}, err
	}
	return stateSnapshot{Data: data, Present: present, identity: identity}, nil
}
...
func (s *unixState) readLeaf(relative string, maximum int64,
	...
	if maximum <= 0 || maximum > maxState || mode.Perm() != mode || mode == 0 {
		return nil, fmt.Errorf("invalid authority leaf bounds")
	}
	if err := s.verifyBindings(); err != nil {
		return nil, err
	}
	...
	if prior, ok := s.authorityLeaves[relative]; ok && (!os.SameFile(prior.info, info) ||
		prior.digest != digest || prior.maximum != maximum || prior.mode != mode) {
		clearBytes(data)
		return nil, fmt.Errorf("authority leaf binding changed")
	}
	s.authorityLeaves[relative] = authorityLeafBinding{digest: digest, info: info, maximum: maximum, mode: mode}
	if err = s.verifyBindings(); err != nil {
		clearBytes(data)
		return nil, err
	}
...
func openLeafParent(root *os.Root, parts []string) (*os.Root, bool, error) {
	...
	for _, component := range parts {
		...
		if err != nil || requireOwnedDirectory(info, privateDir, "authority leaf parent") != nil {
			if next != nil {
				_ = next.Close()
			}
...
func (s *unixState) verifyAuthorityLeaves() error {
	for relative, binding := range s.authorityLeaves {
		data, info, err := readAuthorityLeafBound(s.authority, relative, binding.maximum, binding.mode)
		if err != nil {
			return err
		}
		digest := sha256.Sum256(data)
		clearBytes(data)
		if !os.SameFile(binding.info, info) || subtle.ConstantTimeCompare(binding.digest[:], digest[:]) != 1 {
...
func readStateLeaf(root *os.Root, name string,
	maximum int64) ([]byte, bool, fs.FileInfo, error) {
	file, err := openExistingRegular(root, name, false)
	...
	defer func() { _ = file.Close() }()
	before, err := validateOpenLeaf(file, privateMode, "lifecycle state leaf")
	if err != nil {
...
func readBounded(file *os.File, before fs.FileInfo, maximum int64) ([]byte, error) {
	...
	if err != nil || !sameStableFile(before, after) {
		clearBytes(data)
		return nil, fmt.Errorf("leaf changed while reading")
	}
	return data, nil
}
...
func openExistingRegular(root *os.Root, name string, write bool) (*os.File, error) {
	...
	if before.Mode()&os.ModeSymlink != 0 || !before.Mode().IsRegular() {
		return nil, fmt.Errorf("leaf must be real regular file")
	}
...
func snapshotsEqual(first, second stateSnapshot) bool {
	if first.Present != second.Present || !bytes.Equal(first.Data, second.Data) {
		return false
	}
	if !first.Present {
		return first.identity == nil && second.identity == nil
	}
	return sameStableFile(first.identity, second.identity)
}
