// Package bootstrapgrantauthority implements the authenticated, issuance-only
// bootstrap repository-read profile from ADR-0057. It performs no effect.
...
func validateObjectKey(value string) error {
	...
	if !isASCIISnakeKey(value) {
		return fmt.Errorf("object key %q is not ASCII snake_case", value)
	}
	return nil
}
