...
// Issuer owns a copied Ed25519 seed matching the pinned grant_issue key.
type Issuer struct {
	seed  []byte
	trust *Trust
}

// NewIssuer verifies that seed derives the pinned grant_issue public key.
func NewIssuer(seed []byte, trust *Trust) (*Issuer, error) {
	if trust == nil {
	...
	if isKnownFixturePublicKey(publicKey) {
		return nil, fmt.Errorf("known public fixture issuer key is forbidden at runtime")
	}
	if !constantTimeTextEqual(publicKey, trust.keys["grant_issue"].publicKey) {
		return nil, fmt.Errorf("issuer seed does not match the pinned grant_issue key")
	}
	return &Issuer{seed: append([]byte(nil), seed...), trust: trust}, nil
}

// Close overwrites the in-process seed copy. It does not claim HSM isolation.
func (issuer *Issuer) Close() {
	if issuer == nil {
		return
	}
	clearBytes(issuer.seed)
	issuer.seed = nil
}

func (issuer *Issuer) sign(domain []byte, digest string) (string, error) {
	if issuer == nil || len(issuer.seed) != ed25519.SeedSize {
		return "", fmt.Errorf("issuer is closed or unavailable")
	}
	return signDigest(issuer.seed, domain, digest)
}

func signDigest(seed, domain []byte, digestHex string) (string, error) {
	if len(seed) != ed25519.SeedSize {
		return "", fmt.Errorf("issuer seed must contain exactly %d bytes", ed25519.SeedSize)
	}
