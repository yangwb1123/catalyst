...
type goldenExpected struct {
	...
	CanonicalLocatorJSON        string `json:"canonical_locator_json"`
...
func TestGoldenEvolveRepositoryLocatorEvidenceAdaptation(t *testing.T) {
	fixture := loadGoldenFixture(t)
	if fixture.APIVersion != "forgeos.governance.evolve-repo-locator-evidence-adapter.fixture/v1" {
...
func assertGoldenAdaptation(t *testing.T, adaptation *Adaptation, expected goldenExpected) {
	t.Helper()
	assertGoldenEqual(t, "locator JSON", string(adaptation.LocatorJSON()), expected.CanonicalLocatorJSON)
	...
	assertGoldenEqual(t, "locator digest", adaptation.LocatorSHA256, expected.LocatorSHA256)
...
func loadGoldenFixture(t *testing.T) goldenFixture {
	t.Helper()
	path := filepath.Join("..", "..", "..", "docs", "contracts", "fixtures", "evolve-repo-locator-evidence-adapter-v1.json")
