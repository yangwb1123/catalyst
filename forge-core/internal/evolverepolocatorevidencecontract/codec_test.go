...
func TestDecodeRequestRejectsAdversarialJSON(t *testing.T) {
	...
	tests := []struct{ name, input, want string }{
		...
		{"unknown observation", strings.Replace(raw, `"observation":{`, `"observation":{"alien":null,`, 1), "expected exactly"},
		{"unknown locator", strings.Replace(raw, `"locator":{`, `"locator":{"alien":null,`, 1), "expected exactly"},
		{"root underscore", `{"_format":"x",` + raw[1:], "not allowed canonical snake_case"},
		{"leading whitespace", " " + raw, "not exact compact canonical"},
		...
		{"subjects null", strings.Replace(raw, `"subjects":["evolve:architecture-drift","run:evolve-0050"]`, `"subjects":null`, 1), "must be an array"},
		{"escaped unicode", strings.Replace(raw, "rules", `r\u0075les`, 1), "not exact compact canonical"},
		{"escaped slash", strings.Replace(raw, `.arch/rules.yaml`, `.arch\/rules.yaml`, 1), "not exact compact canonical"},
		{"bidi", strings.Replace(raw, `"thorough"`, `"bad\u202e"`, 1), "forbidden Unicode"},
	}
...
func TestCanonicalCodecPreservesAllowedUnicodeAndHTMLCharacters(t *testing.T) {
	request := validRequest()
	request.Observation.Locator.Path = "架构/<规则>&.yaml"
	request.Observation.Locator.Detail = "架构规则 < 预算 & 可追溯 >"
	encoded, err := canonicalRequestJSON(request)
	if err != nil {
		t.Fatalf("canonicalRequestJSON: %v", err)
	}
	for _, want := range []string{"架构/<规则>&.yaml", "架构规则 < 预算 & 可追溯 >"} {
		if !bytes.Contains(encoded, []byte(want)) {
			t.Fatalf("canonical bytes escaped allowed text %q: %s", want, encoded)
		}
	}
