...
func TestAgentExecutor_EchoEmitsNoCostEvent(t *testing.T) {
	...
	ex := agentExecutor(runOpts{executor: "command", agentCmd: "echo", root: t.TempDir()}, func(string) {}, costSink)
...
func TestAgentExecutor_ClaudeGetsOutputFormatJSON(t *testing.T) {
	ex := agentExecutor(runOpts{executor: "command", agentCmd: "claude", agentPermission: "acceptEdits", root: t.TempDir()}, func(string) {}, nil)
...
func TestAgentExecutor_ClaudeObserveDrivesCostSink(t *testing.T) {
	...
	ex := agentExecutor(runOpts{executor: "command", agentCmd: "claude", root: t.TempDir()}, func(string) {}, costSink)
	ce := ex.(orchestrator.CommandExecutor)
	if ce.Observe == nil {
